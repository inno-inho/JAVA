...
public class C11RestRequestResponseMain {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		HttpRequest httpRequest = HttpRequest.newBuilder().uri(URI.create(url)).GET().build();		
									// HttpRequest.newBuilder().uri(URI.create(url)).GET()는 객체생성을 위한 코드
...
//		for(int i = 0;i < data.size();i++) {
//			JsonNode item = data.get(i);
//			System.out.println(item.get("BZ_NM"));		// BZ_NM키의 value값에 해당하는걸 꺼내오겠다
//														// BZ_NM -> 식당 이름
//		}
		...
		System.out.println("status: " + responseObject.total);
	...
	// InnerClass
			private static class Data{
				...
				public String toString() {
					return "Data [cnt=" + cnt + ", oPENDATA_ID=" + oPENDATA_ID + ", gNG_CS=" + gNG_CS + ", fD_CS="
							+ fD_CS + ", bZ_NM=" + bZ_NM + ", tLNO=" + tLNO + ", mBZ_HR=" + mBZ_HR + ", sEAT_CNT="
							+ sEAT_CNT + ", pKPL=" + pKPL + ", hP=" + hP + ", pSB_FRN=" + pSB_FRN + ", bKN_YN=" + bKN_YN
							+ ", iNFN_FCL=" + iNFN_FCL + ", bRFT_YN=" + bRFT_YN + ", dSSRT_YN=" + dSSRT_YN + ", mNU="
							+ mNU + ", sMPL_DESC=" + sMPL_DESC + ", sBW=" + sBW + ", bUS=" + bUS + "]";
				}
